...
function M.setup(config)
  ...
  config.keys = {
    ...
    { key = '&', mods = 'SHIFT|CTRL', action = act.ActivateTab(6) },
    {
      key = '\'',
      mods = 'SHIFT|ALT|CTRL',
      action = act.SplitVertical { domain =
      'CurrentPaneDomain' }
    },
    ...
    --#region Prioritize PWSH jump to prompts
  ...
  config.key_tables = {
    copy_mode = {
      ...
      { key = 'Escape', mods = 'NONE',  action = act.CopyMode 'Close' },
      {
        key = 'Space',
        mods = 'NONE',
        action = act.CopyMode { SetSelectionMode =
        'Cell' }
      },
